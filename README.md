...
**`propagate_balance()`** evaluates each row and column as a unit. Once a line reaches its quota for one symbol, every remaining undecided cell in that line is forced to the other symbol. It also performs two lookaheads that test a hypothetical assignment before committing to it. The first is a one-cell lookahead: when a line sits exactly one cell short of its quota, the function checks, for each undecided cell, whether assigning it the quota-completing symbol would force the rest of the line to the opposite symbol in a way that creates a three-in-a-row; if so, that cell is ruled out for that symbol. The second is the same idea applied to a pair of cells linked by an equals connector, since both must take the same value: when a line sits exactly two cells short of its quota, the function checks whether assigning that symbol to both cells of an undecided equals pair would, once it forces the rest of the line, create a three-in-a-row; if it would, the pair is locked to the opposite symbol instead. This second lookahead catches contradictions the first one misses, since a quota deficit of two never triggers a single-cell test, and the resulting three-in-a-row can land anywhere in the line, not just next to the pair.
